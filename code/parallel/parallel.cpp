		...
		float Tp_sec, float Ts_sec, float Wp, float Tp, float Ws, float Ts, String filename)
{
    ...
    //Gflop/s
    float gflops = (Pk_counters[0] + Pk_counters[1])*1e-9 / Tp_sec; 
    ofile << "Performance (Gflops/s):  "<<gflops<<endl;
    //percent of peak
    ofile << "frac peak perf:          "<<gflops/3340*100<<"%"<<endl;
    //Traffic
...
{   
   ...
   if(ret3 != PAPI_OK){
	...
	auto vec = gtiff.GetRasterBand(1);
	lst_Nr = gtiff.GetDimensions()[0]*sNr;
	lst_Nc = gtiff.GetDimensions()[1]*sNc*16;
	vector<float> v(lst_Nr*lst_Nc);
	memcpy(v.data(), vec.data(), vec.size()*sizeof(float));
	Image img(v, lst_Nr, lst_Nc);
    ...
    for(auto &yearv : data.images){
		...
		lst_Nc = dayv.second.Ncol;
                memcpy(&Pk[lst_Nr*lst_Nc*nt], dayv.second.vec.data(), lst_Nr*lst_Nc*sizeof(float));
    ...
    write_analysis(Pk_counters, Pk_counters_serial, Tp, rel_times, Tp_sec, Ts_sec, W, Tp, W, Ts, outfile);
