...
void inline right_transform(float *img_data, const int &Nr, const int &Nc, const int &Nt)
{
    float row_tmp[Nc];
...
void inline left_transform(float *img_data, int &Nr, int&Nc, int &Nt)
{
    float mat_tmp = (float*)malloc(Nr*Nc*sizeof(float));
    memset(mat_tmp,0,sizeof(float)*Nc*Nr);
    int chunk = 32768/(2.0*sizeof(double)); //ACJ chunk size to fit in cache
    ...
    for(int T=0; T<Nt; T++)
    {
        ...
        { 
            ...
            {   
                ...
                while(start<N)
                {
                    for(size_t k=0;k<Nr;n++)
                    {
                        double f_mk = F(cos,m,k)
...
//done profiling.
void inline get_Pk_kernel(float* img_data, const int &Nr, const int &Nc, const int &Nt)
{
